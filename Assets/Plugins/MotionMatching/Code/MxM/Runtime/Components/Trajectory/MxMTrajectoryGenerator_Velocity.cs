...
namespace MxM
{
    ...
    *********************************************************************************************/
    ...
    public class MxMTrajectoryGenerator_Velocity : MxMTrajectoryGeneratorBase
    {
        ...
        float acctime = 0, w = 0, totalDistance = 0;
        ...
        private Vector3 parametricCircle(float t, float r, float v)
        {
            // This is just a simplified version of the circle equation
            // t -> time
            // r -> radius
            // v -> linear speed
            // w -> angular speed
            // (t/(2pi*r))/v -> t/(r/v) -> (t*v)/r
            // w = v / r
            // derivative of r*cos(w*t) -> - r*w*sin(wt)

            float w = v / r;

            //Debug.Log(t);
            float x = -r * w * Mathf.Sin(w * t);
            float y = r * w * Mathf.Cos(w * t);

            return new Vector3(x, 0, y);
        }

        private void plannedTrajectory(float t, float r, float v)
        {
            float w = v / r;
            ...
            planned_pos.Add(x.ToString() + ",0," + y.ToString());
        }
        ...
        *********************************************************************************************/
        protected virtual void Start()
        {
            ...
            gameObject.transform.position = new Vector3(Radius, 0, -10);
        ...
        *********************************************************************************************/
        private Vector3 CalculateDesiredLinearVelocity()
        {
            ...
            */
            ...
            //Debug.Log(debugger.DebugData.GetDebugFrame(frame_Count).playableStates[0].AnimId);

            if(has_inertia && (transform.position.z < 0))
            {
                InputVector = new Vector3(0, 0, LinearSpeed) / m_maxSpeed;
            }
            else
            {
                if (acctime * (w * Mathf.Rad2Deg) < 360)
                {
                    if (has_inertia)
                    {
                        has_inertia = false;
                    ...
                    InputVector = parametricCircle(acctime, Radius, LinearSpeed) / m_maxSpeed;
                    plannedTrajectory(acctime, Radius, LinearSpeed);
                    finalTrajectory(acctime, gameObject);
                    for (int i = 0; i < 8; i++)
                    {
