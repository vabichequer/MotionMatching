...
namespace MxM
{
    ...
    *********************************************************************************************/
    ...
    public class MxMTrajectoryGenerator_FlowField : MxMTrajectoryGeneratorBase
    {
        ...
        [Header("Mods")]
        public GameObject FlowField_ref; //A reference to the flowfield transform
        ...
        *********************************************************************************************/
        ...
        {
            GameObject nearest_arrow = FlowField_ref.GetComponent<FlowField>().SearchNearestArrow(gameObject.transform.position);
            float speed = FlowField_ref.GetComponent<FlowField>().GetArrowSpeed(int.Parse(nearest_arrow.name));
            float angle = 270 - (nearest_arrow.transform.rotation.eulerAngles.y);

            float Vz = speed * Mathf.Sin(angle * Mathf.Deg2Rad);
            float Vx = speed * Mathf.Cos(angle * Mathf.Deg2Rad);

            Debug.Log("Vz: " + Vz + " Vx: " + Vx + " Angle: " + angle + " Speed: " + speed + " Sin: " + Mathf.Sin(angle) + " Cos: " + Mathf.Cos(angle));

            //Get the movement input
            InputVector = new Vector3(Vx, 0f, Vz);
